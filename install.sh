...
deckard_bootstrap() (
  ...
  if [ "$yes" = false ]; then
    ...
    printf 'Install Deckard v0.4.0 (including model weights) and configure shell %s? [y/N] ' "$shell_choice" >&3
    answer=
  ...
  archive="$work/deckard-v0.4.0-macos-arm64.tar.gz"
  url='https://github.com/sgoedecke/deckard/releases/download/v0.4.0/deckard-v0.4.0-macos-arm64.tar.gz'
  printf 'Downloading Deckard v0.4.0…\n'
  curl --fail --location --proto '=https' --proto-redir '=https' \
    --connect-timeout 30 --max-time 1800 --output "$archive" "$url" ||
    fail 'Download failed. The pinned v0.4.0 release must be published before this installer can be used.'
